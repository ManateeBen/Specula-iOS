...
import { getTextConfig, getVisionConfig } from './settings.service'
...
function buildImagePrompt(req: ImageExplainNeedRequest): string {
  const tone = req.tone === 'casual' ? '轻松、自然，但不油滑' : '严谨、清晰、克制'
  const followUpRule = req.followUp
    ? `这是一次追问。追问内容：${req.followUp}\n直接回应追问，并保持与前文一致。`
    : ''
  return `用户正在阅读《${req.bookTitle || '当前书籍'}》的“${req.chapterTitle || '当前章节'}”，希望理解一张图片。

图片替代文本：${req.altText || '无'}
图片标题：${req.caption || '无'}
图片附近正文：${req.context || '无'}

先准确识别图片中的对象、文字、箭头、流程、空间关系或数据，再按所选需求讲解。图片与附近正文是唯一事实依据；看不清或无法从图中确定的内容必须明确说明，不得补造。
${followUpRule}

${NEED_INSTRUCTIONS[req.need]}

只输出一个合法 JSON 对象：{"sections":[{"label":"6字内小节标签","text":"段落内容"}],"tail":{}}
sections 为 2-3 节，总字数不超过 250 个汉字；可用 <b> 标记关键短语，每节至多一处。语气：${tone}。不要输出 Markdown 代码围栏。`
}
...
export async function explainImageNeed(req: ImageExplainNeedRequest): Promise<StructuredExplanation> {
  const imageIdentity = `image:${hashText(req.imageDataUrl)}:${req.altText}:${req.caption}:${req.context}`
  const selectionHash = getSelectionHash(imageIdentity, req.followUp)
  const cached = queryOne<{ result_json: string }>(
    `SELECT result_json FROM ai_explanation_cache WHERE book_id = ? AND selection_hash = ? AND need = ? AND tone = ?`,
    [req.bookId, selectionHash, req.need, req.tone]
  )
  if (cached) {
    try {
      const result = JSON.parse(cached.result_json) as StructuredExplanation
      track(req.bookId, req.chapterId, 'ai_image_explanation_cache_hit', { need: req.need, tone: req.tone })
      return { ...result, fromCache: true }
    } catch { /* regenerate a corrupt cache row */ }
  }

  const config = await getVisionConfig()
  if (!config.apiKey) throw new Error('应用未配置内置视觉模型凭据')
  const client = new OpenAI({ apiKey: config.apiKey, baseURL: config.baseURL, dangerouslyAllowBrowser: true })
  const response = await client.chat.completions.create({
    model: config.model,
    messages: [
      { role: 'system', content: '你是阅读应用 Specula 的视觉讲解编辑。严格遵守用户给出的 JSON 协议，并只依据图片与所给上下文。' },
      {
        role: 'user',
        content: [
  ...
  track(req.bookId, req.chapterId, 'ai_image_explanation_generated', {
    need: req.need,
    tone: req.tone,
    fallback: result.fallback,
    tokens: response.usage?.total_tokens || null,
